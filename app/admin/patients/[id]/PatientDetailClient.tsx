...
import { deletePatient, deleteConsultation } from "@/lib/actions";
import {
  ArrowLeft,
  Calendar,
  Clock,
...
export function PatientDetailClient({
  ...
  const handleDeleteConsultation = () => {
    ...
    startTransition(async () => {
      const res = await deleteConsultation(consultationToDelete.id);
      if (res.success) {
                  ...
                  >
                    <div className="flex justify-between items-start pb-2 border-b border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          onClick={() => setConsultationToDelete(consult)}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="max-w-sm w-full p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              This will permanently delete this patient, all scheduled and past appointments, consultation records, prescriptions, and entire medical history. This action cannot be undone.
            </p>
          ...
          <div className="max-w-sm w-full p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col gap-4">
            <div className="flex items-center gap-3 text-red-600 dark:text-red-400">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Delete consultation record from {new Date(consultationToDelete.createdAt).toLocaleDateString("en-IN")}
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-600 dark:text-slate-400">
              This consultation record, diagnosis, notes, and prescriptions will be permanently deleted.
            </p>
